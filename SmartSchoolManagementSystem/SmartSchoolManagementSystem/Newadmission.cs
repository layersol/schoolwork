        ...
        private void dvgstudent_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                var q = (from a in db.TBLADDMISSIONs
                         join b in db.TBLSTUDENTENRROLs on a.SID equals b.SystemId
                         join c in db.Tblclasssections on b.ClassId equals c.ID
                         where a.SID == dgva
                         select new { main=a, two=c });
               // txtsystem.Text =Convert.ToString();
